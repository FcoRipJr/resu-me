...
const exampleCandidate = {
  ...
  objective: 'Buscar oportunidades em desenvolvimento web com foco em experiência do usuário.',
  ...
  education: [{ institution: 'Universidade de São Paulo', degree: 'Bacharel em Sistemas de Informação' }],
  certifications: ['AWS Cloud Practitioner'],
  languages: ['Português', 'Inglês'],
  projects: [{ name: 'Portal de Clientes', description: 'Plataforma de autoatendimento em React.' }],
};
...
const Optimize = ({ language, t }) => {
  const [candidateJson, setCandidateJson] = useState(JSON.stringify(exampleCandidate, null, 2));
  ...
  const promptPreview = useMemo(() => {
    try {
      ...
      const result = generatePrompt(parsed, {
        text: jobText,
        url: jobUrl,
  ...
  const handleGeneratePrompt = () => {
    try {
      ...
      setPrompt(generatePrompt(parsed, { text: jobText, url: jobUrl, language: selectedLanguage }, jobMode));
      setError('');
    } catch (generateError) {
      ...
      <div className="panel">
        ...
        <div className="field-group">
          ...
          <div className="radio-row">
            <label>
              <input type="radio" checked={jobMode === 'text'} onChange={() => setJobMode('text')} />
              {t.optimize.text}
            </label>
            <label>
              <input type="radio" checked={jobMode === 'url'} onChange={() => setJobMode('url')} />
              {t.optimize.url}
            </label>
          </div>
        </div>
        ...
        {jobMode === 'text' ? (
          <div className="field-group">
            ...
            <textarea id="job-text" value={jobText} onChange={(event) => setJobText(event.target.value)} rows={8} />
          </div>
        ) : (
          <div className="field-group">
            <label htmlFor="job-url">{t.optimize.jobUrl}</label>
            <input id="job-url" type="url" value={jobUrl} onChange={(event) => setJobUrl(event.target.value)} />
          </div>
        )}
        ...
        <div className="action-row">
          <button type="button" onClick={handleGeneratePrompt}>{t.optimize.generatePrompt}</button>
          <button type="button" className="secondary" onClick={copyPrompt}>{t.optimize.copyPrompt}</button>
        </div>
